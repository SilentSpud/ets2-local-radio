        ...
        if (!controlRemote) {
          ...
          if (data.action == "goToFavourite") {
            if (g_favourites[g_current_country] != "" && g_current_country != null) {
              ...
              var index = stations[chosen_country]
                .map(function (e) {
                  return e.name;
                })
...
function refresh(data) {
  ...
  if (
    !(
      data.TruckValues.Positioning.HeadPositionInWorldSpace.X == 0.0 &&
      data.TruckValues.Positioning.HeadPositionInWorldSpace.Y == 0.0 &&
      data.TruckValues.Positioning.HeadPositionInWorldSpace.Z == 0.0
    ) ||
    g_show_all
  ) {
    for (var i = 0; i < cities.length; i++) {
      ...
      var distance = Math.sqrt(
        Math.pow(data.TruckValues.Positioning.HeadPositionInWorldSpace.X - cities[i]["x"], 2) + Math.pow(data.TruckValues.Positioning.HeadPositionInWorldSpace.Z - cities[i]["z"], 2)
      );
    ...
    if (
      !available_countries.hasOwnProperty(g_current_country) ||
      ...
      $.getJSON(g_api + "/favourite/" + country_best_reception, function (favourite_lowest_distance) {
        if (favourite_lowest_distance["Name"] != "") {
          index = stations[country_best_reception]
            .map(function (e) {
              return e.name;
            })
...
function setRadioStation(url, country, volume) {
  ...
  var index = stations[g_current_country]
    .map(function (e) {
      return e.url;
    })
    .indexOf(g_current_url);

  $.get(g_api + "/station/" + encodeURIComponent(stations[country][index].name) + "/" + calculateReception(g_countries[country].whitenoise) + "/?" + getFullLogoUrl(stations[country][index].logo));
}
...
function setFavouriteStation(country, name) {
  if (controlRemote) {
    ...
    $.get(g_api + "/favourite/" + country + "/" + encodeURIComponent(name), function () {
      $("#snackbar")
        .html(country_properties[country].name + " <i class='fa fa-heart'></i>  " + name)
        .addClass("show");
      setTimeout(function () {
...
function calculateReception(whitenoise) {
  ...
  if (reception < 0.05) {
    reception = "5";
  } else if (reception < 0.2) {
    reception = "4";
  } else if (reception < 0.35) {
    reception = "3";
  } else if (reception < 0.5) {
    reception = "2";
    ...
    for (var j = 0; j < stations[key].length; j++) {
      ...
      if (typeof stations[key][j]["relative_radius"] === "undefined" || g_countries[key]["distance"] / stations[key][j]["relative_radius"] < g_skinConfig.radius) {
        ...
        content +=
          ...
          '<div class="thumbnail ' +
          (g_current_url == stations[key][j]["url"] && g_current_country == key ? "thumbnail-selected" : "") +
          '" href="#" onclick="setRadioStation(\'' +
          stations[key][j]["url"] +
          "'," +
          " '" +
          key +
          ...
          " '" +
          volume +
          "'); document.getElementById('player').play(); event.preventDefault();\">" +
          '<div class="frame text-center"><div class="station-image-container"><img src="' +
          getFullLogoUrl(stations[key][j]["logo"]) +
          '"></div><br>' +
          '<h3 class="station-title overflow">' +
          stations[key][j]["name"] +
          "</h3>" +
          '<span class="overflow station-subtitle">' +
          ...
          (g_favourites.hasOwnProperty(key) && g_favourites[key] == stations[key][j].name
            ? ""
            : '<button class="btn btn-default btn-xs top-right lang-make-favourite" onclick="setFavouriteStation(\'' +
              key +
              "', '" +
              stations[key][j]["name"] +
              "'); $(this).css('background-color', '#2ebb1e');\">" +
              (typeof g_translation !== "undefined" && typeof g_translation["web"]["make-favourite"] !== "undefined" ? g_translation["web"]["make-favourite"] : "Make favourite") +
              "</button> ") +
          "</div>";
      }
    }
